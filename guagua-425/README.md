# 呱呱 Codex V2 桌宠 | Guagua Codex V2 Pet

《洛克王国》2011 年页游原版 #425的非官方桌宠设计。动作包括托腮想点子、打拳修炼、尴尬失败与得意成功。

An unofficial pet based on the 2011 original web-game form #425 of *Rock Kingdom*. It features chin-resting ideas, martial-arts practice, awkward failure, and proud success.

外观：明黄色中式斜襟短衫、白色盘扣、角色自身右侧打结的青绿色宽腰封，以及蓝色灯笼短裤。

Appearance: A bright yellow wrap shirt, white toggle, wide teal sash tied on the character own right, and blue bloomers.
...
将 `guagua-425` 文件夹复制到 Codex 的 `pets` 目录，然后在 Codex Desktop 的 **Settings → Pets** 中选择“呱呱”。本项目机器上的安装路径是 `E:\AI Agents\Codex\pets\guagua-425`。若刚替换图集，完全退出并重新打开 Codex 或切换到另一只宠物再切回。

Copy `guagua-425` into your Codex `pets` directory, then select **Guagua** in **Settings → Pets**. Restart Codex or switch pets and back if the old atlas remains cached.
