# 逍遥呱呱 Codex V2 桌宠 | Xiaoyao Guagua Codex V2 Pet

《洛克王国》葫芦藤修行形态 #428的非官方桌宠设计。动作包括背负大型双节葫芦，在云团上跑步、练平衡与轻功。

An unofficial pet based on the gourd-vine form #428 of *Rock Kingdom*. It features a large double-lobed gourd, cloud-running, balance training, and light-footed jumps.

外观：沿用原版黄衣蓝裤，青绿色腰封结在角色自身右侧；单条棕色背带从自身右肩斜跨至左腰，背负大型朱橙色双节葫芦。

Appearance: The original yellow shirt and blue shorts, teal sash tied on the character own right, and a single brown strap from the right shoulder to the left waist carrying a large orange-red double-lobed gourd.
...
将 `guagua-428` 文件夹复制到 Codex 的 `pets` 目录，然后在 Codex Desktop 的 **Settings → Pets** 中选择“逍遥呱呱”。本项目机器上的安装路径是 `E:\AI Agents\Codex\pets\guagua-428`。若刚替换图集，完全退出并重新打开 Codex 或切换到另一只宠物再切回。

Copy `guagua-428` into your Codex `pets` directory, then select **Xiaoyao Guagua** in **Settings → Pets**. Restart Codex or switch pets and back if the old atlas remains cached.
